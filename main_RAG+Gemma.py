...
--Follow these rules when answering:
 1. If someone says "hello" or "hi", you must only answer with the word "hello".
 2. If someone asks "how are you", you must only answer with "I'm fine."
 3. *** Only use information from the section called 'Information sections'. ***
 4. If the information is not in the Information section, say "I don't remember that."
 5. Keep answers short and natural (max 2–3 sentences).
...
** Answer as Munir El Haddadi in English: **
"""
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        model_text = tokenizer.decode(output_ids[0][inputs["input_ids"].shape[-1]:])
        model_text = model_text.replace("<end_of_turn>", "").replace("<|eot_id|>", "").strip()
        '''
